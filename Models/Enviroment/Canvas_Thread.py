...
class Canvas_Thread(threading.Thread):
    ...
    def run(self):
        ...
        if(self.id=="Game_Start"):
            #背景用執行緒
            self.BG=BackGround(self.Canvas, self.X, self.Y, self.Right_X, self.Bottom_Y, id="BackGround",img=self.Background_Png)
            self.Thread_Ready(self.BG)
            #主角用執行緒
            self.Player_Born=Player(self.Canvas,self.X,self.Y,self.Right_X,self.Bottom_Y,id="Player",img=self.Player_Png)
        ...
        while self.Alive and self.id=="Game_Start":
            ...
            for enemy in range(len(self.Enemy_List)):
                ...
                if(self.Enemy_List[enemy].Alive==True):
                    ...
                    if (self.Collision.Is_Collision(Object_X=self.Enemy_List[enemy].X,
                                                    ...
                                                    Collision_X=self.Player_Born.X, Collision_Y=self.Player_Born.Bottom_Y,
                    ...
                    for bullet in range(len(self.Bullet_List)):
                        ...
                        if (self.Bullet_Collision.Is_Collision(Object_X=self.Bullet_List[bullet].X,
                                                        ...
                                                        Collision_X=self.Enemy_List[enemy].X, Collision_Y=self.Enemy_List[enemy].Bottom_Y,
            ...
            if (self.Player_Born.Shoot == True):
                ...
                Bullet_Born = Bullet(self.Canvas, self.Player_Born.X,
                                     (self.Player_Born.Bottom_Y - (self.Player_Born.Height) * 2) + self.Player_Born.Height, self.Right_X,
                                     self.Bottom_Y - self.Player_Born.Height, id="Player_Bullet", img=self.Player_Bullet_Png)
                #開始子彈執行緒
